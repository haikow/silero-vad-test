...
push / PR 自动触发两个 job, 各跑同一套 pytest 用例 (2026-09-21 起接入, 52 条)。
...
- **噪声**: 白/粉/褐合成噪声两模型均值都 <=0.06, 误报无忧 (真实环境声是 Tier 2, 需提交素材)
- **SNR**: int8 20/10/5dB 均值 0.93/0.92/0.89, **5dB 下检出覆盖仍 93%** —— 移植蓝本抗噪性极强;
  uint8 0.68/0.56/0.34 深噪声渐退。嘈杂环境产品底线 = int8 在 5dB 仍可检出
