...
### 原厂 WebRTC VAD 对比 (替换论证, 2026-09-22)

原厂 VAD = 闭源 `libwq_sw_vad.a` (Xtensa HiFi5, Mac 不可链接), 算法本体为开源 WebRTC VAD。
用同源开源实现(webrtcvad, 官方 C 源码)在**同一组场景**上对比 (`python ci/compare_webrtc.py`,
需 `pip install webrtcvad 'setuptools<81'`; Silero 侧读 scenario_metrics.json):

| 场景 | WebRTC(最佳档) | int8 (v4) | uint8 (v5) |
|---|---|---|---|
| 干净语音检出 (smoke) | 84% | **95%** | ~78%(均值) |
| 噪声误报: 合成白噪 / 咖啡馆 / 雨声 | **98~100% / 100%全程误开 / 36~62%** | 均值 0.02~0.10, 四件套 **0 段** | 9 段/60s(咖啡馆) |
| 白噪 5dB SNR 检出 | 48~62% | **93%** | 30% |
| 咖啡馆 0dB SNR 检出 | 75%(96%@a0 系误报掺水) | **98%** | 2% |
| 电平鲁棒 (peak 0.05) | 34~78% (a3 塌) | **89%** | 48% |

结论: **原厂 WebRTC VAD 在真实环境声上误报失控(咖啡馆全程误开), 噪声下检出率也全面落后;
int8 Silero 每个维度第一**。"雨中旁白 WebRTC 96% 检出"为掺水数字(裸雨声它都误开 36~62%)。
注意: 此为开源同源代理的结论, 物奇若改过算法或固件用不同激进度档位会有出入;
拿到 `libwq_sw_vad.a` 后可用本地 XtensaTools 的 xt-run 仿真器跑真库复核。
